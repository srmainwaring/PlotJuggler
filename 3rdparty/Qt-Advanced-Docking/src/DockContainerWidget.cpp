	...
	QWidget*& CreatedWidget, bool Testing)
{
	...
	while (s.readNextStartElement())
	{
		...
		bool Result = true;
		if (s.name() == QLatin1StringView("Splitter"))
		{
			Result = restoreSplitter(s, ChildNode, Testing);
		}
		else if (s.name() == QLatin1StringView("Area"))
		{
			Result = restoreDockArea(s, ChildNode, Testing);
		}
		else if (s.name() == QLatin1StringView("Sizes"))
		{
	...
	QWidget*& CreatedWidget, bool Testing)
{
	...
	while (s.readNextStartElement())
	{
		if (s.name() != QLatin1StringView("Widget"))
		{
			continue;
		}
...
bool CDockContainerWidget::restoreState(CDockingStateReader& s, bool Testing)
{
	...
	if (IsFloating)
	{
		...
		if (!s.readNextStartElement() || s.name() != QLatin1StringView("Geometry"))
		{
			return false;
		}
